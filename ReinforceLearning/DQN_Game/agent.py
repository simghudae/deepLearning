...
class DQNAgent:
    def __init__(self, screenHeight, screenWidth, nAction, learningRate, updateRate, maxEpoch):
    ...
    def _buildNetwork1(self):
        modelDQN = tf.keras.Sequential([
            tf.keras.layers.Reshape(target_shape=(self.screenHeight, self.screenWidth, 1), input_shape=(self.screenHeight, self.screenWidth)),
            tf.keras.layers.Conv2D(32, kernel_size=(3, 3), padding='same', activation='relu', kernel_initializer='glorot_normal'),
            tf.keras.layers.Conv2D(32, kernel_size=(3, 3), padding='same', activation='relu', kernel_initializer='glorot_normal'),
        ...
        modelDQN.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=self.learningRate), loss=tf.keras.losses.CategoricalCrossentropy(), metrics=['accuracy'])
        return modelDQN
    ...
    def getAction(self, state, epoch, training=True):
        if training:
            getnoiseQvalue = ((self.maxEpoch - epoch) / self.maxEpoch) * np.random.uniform(size=[1, self.nAction]) + (epoch / self.maxEpoch) * self.mainQ.predict(x=state)
